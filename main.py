...
for index, wallet_key_pair in enumerate(tqdm(keys_list_splitted[:])):
    ...
    while True:
        ...
        if sol_balance>SOL_MIN_BALANCE_TO_TRADE:
            ...
            main.debug(f'wallet {acc_nickname} has usdt, start buying usdc')
            volume = buy_usdc(client, spot_account_id, usdt_balance)
            if volume == 0:
                error_counter +=1
                continue
            actual_turnover += volume
            ...
            time.sleep(1) 
            item = get_usdt_usdc_sol_balances(client,spot_account_id, acc_nickname)
            main.debug(f'wallet {acc_nickname} state after buying usdc (operation number {trades_counter}): '+str(item))
            # print(item)
        
        elif usdc_balance>USDT_MIN_BALANCE_TO_TRADE:
            main.debug(f'wallet {acc_nickname} has usdc, start buying usdt')
            # print('sell usdc')
            volume = sell_usdc(client, spot_account_id, usdc_balance)
            if volume == 0:
                error_counter +=1
                continue
            actual_turnover += volume
